...
def riemannian_sgd(train_x, train_y, vectorized_tt_dot_h, loss_h,
                   ...
                   debug=False, beta=0.5, rho=0.1, l1_reg=0.):
    ...
    for e in xrange(start_epoch, num_passes):
        ...
        for batch_idx in xrange(num_batches):
            ...
            lasso = tt.multifuncrs([w], lambda x: np.abs(x), 1e-6)
            lasso.round(eps=0,rmax=30)
            if (l1_reg > 1e-14):
                lasso_subgradient = tt.multifuncrs([w], lambda x: pywt.threshold(x, l1_reg, "soft"), 1e-14)
            else:
                lasso_subgradient = 0 * w
            batch_loss = np.sum(batch_loss_arr) + reg * wreg_wreg / 2.0 + l1_reg * tt.core.tools.sum(lasso * reg_tens)
            ...
            direction = 0.5 * riemannian.project(w, [direction, reg * wregreg, lasso_subgradient])
           ...
           #                                      batch_gradient_b, reg, wreg_wreg,
            #                                     wreg_dir, dir_dir, lasso_subgradient)
            #        return obj
...
#           Armiho step choosing.
            ...
            if ((newgradient != None) and ((direction - newgradient).norm() < 0.0000001)):
                print ("A")
                return w, b
            newgradient = direction
            w = direction.round(rmax = 30, eps=0)
